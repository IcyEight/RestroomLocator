...
$(document).ready(function () {
   if ($('form').length > 0) {
    ...
    // This function will be executed when the form is searchted
    function FormSearch() {
       $.searchForm = true;
       if (!$('form').valid()) {
         return false;
         ...
         if ($("#Address").data("IsChecking") == true) {
            $("#Address").data("SearchForm", true);
           return false;
         }

         alert('Form is valid!');
         // return true;   // Uncomment to search the form.
         return false;     // Supress the form submission for test purpose.
       }
     }

     // Attach the FormSearch function to the Search button
     if ($('#Search').exists()) {
       $("#Search").click(FormSearch);
     }

     // Execute the ForumSearch function when the form is searchted
...
//     being searchted, the code below needs to resume that search. 
...
//     // We're no longer in the midst of validating
     ...
     /* This code is being run after the validation for this field, if the form was being searchted before this validtor was called then we need to re-search the form. */
     if ($(element).data("SearchForm") == true) {
       form.search();
     } else {
